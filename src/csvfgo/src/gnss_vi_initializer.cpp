...
// GNSSVIInitializer::GNSSVIInitializer( const std::vector<std::vector<double>> gpsresult, 
//      const std::vector<std::vector<double>> gpsresult_buf,  const std::vector<double> gpsresult_buf_)
// {
//     num_all_meas = 0;
//     all_sat_states.clear();
//     for (uint32_t i = 0; i < gpsresult.size(); ++i)
//     {
//         num_all_meas += gpsresult[i].size();
//         all_sat_states.push_back(sat_states(gpsresult[i], gpsresult_buf[i]));
//     }
// }
...
{
    std::printf("yaw align ... "); std::fflush(stdout);
    aligned_yaw = 0;
    rcv_ddt = 0;
